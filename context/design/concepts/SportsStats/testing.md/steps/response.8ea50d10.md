---
timestamp: 'Thu Oct 16 2025 23:08:35 GMT-0400 (Eastern Daylight Time)'
parent: '[[../20251016_230835.d4dcaab1.md]]'
content_id: 8ea50d10d0779954ab16b990881c7735b29903ebd8bf7e649e10a6867dae1bb0
---

# response:

You've hit on a common and slightly frustrating TypeScript challenge when working with union types that include an error object! The issue arises because `assertEquals` (or similar assertion functions) often expect a specific type, and when you pass a union like `{ sport: ID } | { error: string }`, TypeScript gets confused about which branch of the union to validate against, especially when you're trying to check for the *absence* of a property like `error`.

The solution is to create more robust type-guarding helper functions for both success and failure scenarios. These functions will explicitly narrow the type based on the presence (or absence) of the `error` property, allowing TypeScript to correctly infer the type in subsequent code.

Here's the updated test suite with two new helper functions, `assertSuccess` and `assertFailure`, that correctly handle these union types and make your tests cleaner and more type-safe:
  ...
  async () => {
    ...
    try {
      ...
      assertFailure(
        ...
        `Stat '${STAT_RUNS}' is already a key stat for sport '${SPORT_CRICKET_NAME}'.`,
      ...
      assertFailure(
        ...
        `Stat '${STAT_ASSISTS}' is not a key stat for sport '${SPORT_CRICKET_NAME}'.`,
      ...
      assertEquals(Object.keys(fetchEmptyStats.keyStatsData).length, 0, "Should return an empty map for no stat values");
